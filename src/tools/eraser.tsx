...
function createEraserHandler(): ToolHandler {
  ...
  function paint(
    ...
    if (layer.format === 'indexed8') {
      ...
      const x0r = Math.round(p0x), y0r = Math.round(p0y)
      const x1r = Math.round(p1x), y1r = Math.round(p1y)
  ...
  return {
    ...
    onPointerUp(_pos: ToolPointerPos, ctx: ToolContext) {
      ...
      if (lastRendered && lastCtrl &&
          Math.hypot(lastCtrl.x - lastRendered.x, lastCtrl.y - lastRendered.y) >= 1) {
        paint(
          lastRendered.x, lastRendered.y,
          lastCtrl.x, lastCtrl.y,
          lastCtrl.x, lastCtrl.y,
...
function EraserOptions({ styles }: { styles: ToolOptionsStyles }): React.JSX.Element {
  ...
  const handleSoftness = (v: number): void => { eraserOptions.softness = v; setSoftness(v) }
  const handleSmoothing = (v: number): void => { eraserOptions.smoothing = v; setSmoothing(v) }
  ...
  return (
    <>
      ...
      <SliderInput value={softness} min={0} max={100} suffix="%" inputWidth={42} onChange={handleSoftness} />
      <span className={styles.optSep} />
