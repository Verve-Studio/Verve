...
export const textOptions = {
  ...
  color: { r: 255, g: 255, b: 255, a: 255 } as { r: number; g: number; b: number; a: number },
}
...
export function getTextBounds(ls: TextLayerState): { x: number; y: number; w: number; h: number } {
  ...
  const textW = Math.max(...lines.map((line) => _measureCtx.measureText(line || 'M').width))
  ...
  return {
    ...
    onPointerUp({ x, y }: ToolPointerPos, ctx: ToolContext): void {
      ...
      const layer: TextLayerState = {
        ...
        letterSpacing: (textOptions.letterSpacing / 1000) * textOptions.fontSize,
      ...
      {open && ReactDOM.createPortal(
        ...
        >
          ...
          <div style={{ overflowY: 'auto', flex: 1 }}>
            {filtered.map((f) => (
              <div
                ...
                style={{
                  ...
                  cursor:       'pointer',
                  color:        '#e0e0e0',
                  background:   f === value ? 'rgba(0,120,255,0.25)' : 'transparent',
                  whiteSpace:   'nowrap',
                  overflow:     'hidden',
                  textOverflow: 'ellipsis',
                }}
            ...
            {filtered.length === 0 && (
              <div style={{ padding: '8px', color: '#888', fontSize: 12 }}>No fonts found</div>
            )}
          </div>
        </div>,
        document.body,
...
function TextOptions({ styles }: { styles: ToolOptionsStyles }): React.JSX.Element {
  ...
  const [strikethrough, setStrikethrough] = useState(activeTextLayer?.strikethrough ?? textOptions.strikethrough)
  ...
  const [lineHeight, setLineHeight]       = useState(activeTextLayer?.lineHeight ?? textOptions.lineHeight)
  const [kerning, setKerning]             = useState<'auto' | 'none'>(activeTextLayer?.kerning ?? textOptions.kerning)
  ...
  useEffect(() => {
    if (activeTextLayer) {
      ...
      const milliems = activeTextLayer.fontSize > 0
        ? Math.round(((activeTextLayer.letterSpacing ?? 0) / activeTextLayer.fontSize) * 1000)
        : 0
  ...
  const applyChange = (patch: Partial<TextLayerState>): void => {
    ...
    if (patch.strikethrough !== undefined) textOptions.strikethrough = patch.strikethrough
    if (patch.align         !== undefined) textOptions.align         = patch.align
    if (patch.lineHeight    !== undefined) textOptions.lineHeight    = patch.lineHeight
  ...
  const handleSize          = (v: number):       void => {
    ...
    if (activeTextLayer && activeTextLayer.boxWidth > 0 && activeTextLayer.boxHeight > 0 && activeTextLayer.fontSize > 0) {
      const scale = v / activeTextLayer.fontSize
      patch.boxWidth  = Math.max(20, Math.round(activeTextLayer.boxWidth  * scale))
      patch.boxHeight = Math.max(20, Math.round(activeTextLayer.boxHeight * scale))
    }
    applyChange(patch)
  }
  ...
  const handleStrikethrough = (v: boolean):      void => { setStrikethrough(v);  applyChange({ strikethrough: v }) }
  ...
  const handleLetterSpacing = (v: number):       void => {
    ...
    const lsPx = (v / 1000) * (activeTextLayer?.fontSize ?? textOptions.fontSize)
    applyChange({ letterSpacing: lsPx })
    textOptions.letterSpacing = v
  }
  const handleLineHeight    = (v: number):       void => { setLineHeight(v);     applyChange({ lineHeight: v }) }
  const handleKerning       = (v: 'auto' | 'none'): void => { setKerning(v);    applyChange({ kerning: v }) }
  ...
  return (
    <>
      ...
      <label className={styles.optLabel} data-text-editor-safe>Size:</label>
      <span data-text-editor-safe><SliderInput value={fontSize} min={6} max={400} inputWidth={46} onChange={handleSize} /></span>
      <span className={styles.optSep} data-text-editor-safe />
      {([
        { label: 'B', active: bold,          title: 'Bold',          style: { fontWeight: 'bold' as const },                                     onToggle: handleBold },
        { label: 'I', active: italic,        title: 'Italic',        style: { fontStyle: 'italic' as const },                                    onToggle: handleItalic },
        { label: 'U', active: underline,     title: 'Underline',     style: { textDecoration: 'underline' as const },                            onToggle: handleUnderline },
        { label: 'S', active: strikethrough, title: 'Strikethrough', style: { textDecoration: 'line-through' as const },                         onToggle: handleStrikethrough },
      ] as const).map(({ label, active, title, style, onToggle }) => (
        <button
          key={label}
      ...
      {showColorPicker && (
        <ColorPickerDialog
          ...
          title="Text Color"
          initialColor={{ r: color.r / 255, g: color.g / 255, b: color.b / 255, a: color.a / 255 }}
          onConfirm={(c) => {
            const newColor = {
