...
function createBrushHandler(): ToolHandler {
  ...
  return {
    onPointerDown(
      { x, y, pressure, timeStamp }: ToolPointerPos,
      ctx: ToolContext,
    ) {
    ...
    onPointerMove(
      ...
      if (Math.hypot(tipX - lastRendered.x, tipY - lastRendered.y) >= spacing) {
        paint(
          ...
          lastRendered.y,
          lastCtrl.x,
          lastCtrl.y, // B-spline attractor → Bézier control point
          tipX,
          tipY,
          prevSize,
          prevOpacity,
...
function BrushOptions({
  ...
  const [velocityTracking, setVelocityTracking] = useState(
    brushOptions.velocityTracking,
  );
  ...
  return (
    <>
      ...
      <label
        ...
        title="Filter out pointer noise — higher values produce cleaner edges at the cost of slight lag"
      >
        Smoothing:
      </label>
      <SliderInput
      ...
      <label
        className={styles.optLabel}
        title="Elongates each dab along the stroke direction — higher values give a smeared/calligraphic feel"
      >
        Motion:
      </label>
      <SliderInput
