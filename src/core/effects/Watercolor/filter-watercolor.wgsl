...
fn sobelMag(pi: vec2i, dims: vec2i) -> f32 {
  let tl = luma(textureLoad(srcTex, clampPx(pi + vec2i(-1, -1), dims), 0).rgb);
  let tc = luma(textureLoad(srcTex, clampPx(pi + vec2i(0, -1), dims), 0).rgb);
  let tr = luma(textureLoad(srcTex, clampPx(pi + vec2i(1, -1), dims), 0).rgb);
  let ml = luma(textureLoad(srcTex, clampPx(pi + vec2i(-1, 0), dims), 0).rgb);
  let mr = luma(textureLoad(srcTex, clampPx(pi + vec2i(1, 0), dims), 0).rgb);
  let bl = luma(textureLoad(srcTex, clampPx(pi + vec2i(-1, 1), dims), 0).rgb);
  let bc = luma(textureLoad(srcTex, clampPx(pi + vec2i(0, 1), dims), 0).rgb);
  let br = luma(textureLoad(srcTex, clampPx(pi + vec2i(1, 1), dims), 0).rgb);
...
fn fs_watercolor(in: AdjVertOut) -> @location(0) vec4<f32> {
  ...
  for (var dy = -r; dy <= r; dy = dy + 1) {
    for (var dx = -r; dx <= r; dx = dx + 1) {
      ...
      let c = textureLoad(srcTex, p, 0).rgb;
  ...
  let edgeMag = sobelMag(coordI, dims);
