...
import type { AddNoiseEffectLayer } from "@/core/effects/AddNoise/AddNoiseEffect";
import { ParentConnectorIcon } from "@/ux/windows/ToolWindowIcons";
import styles from "@/core/effects/_shared/filterPanel.module.scss";

interface Props {
  layer: AddNoiseEffectLayer;
  parentLayerName: string;
}

export function AddNoisePanel({
  layer,
  parentLayerName,
    ...
    <div className={styles.content}>
      <div className={styles.row}>
        ...
        <div className={styles.trackWrap}>
          <input
            ...
            value={amount}
            style={
              { "--pct": String((amount - 1) / 399) } as React.CSSProperties
            }
        ...
        <input
          ...
          onChange={(e) => {
            ...
            if (!isNaN(v))
              up({ amount: Math.min(400, Math.max(1, Math.round(v))) });
          }}
        />
      ...
      <div className={styles.row}>
        ...
        <div className={styles.segmented}>
          {(["uniform", "gaussian"] as const).map((d) => (
            <button
              key={d}
              className={`${styles.segBtn} ${distribution === d ? styles.segBtnActive : ""}`}
              onClick={() => up({ distribution: d })}
            >
      ...
      <div className={styles.footer}>
        ...
        <button
          ...
          onClick={() =>
            up({ amount: 25, distribution: "gaussian", monochromatic: false })
          }
          title="Reset"
        >
          Reset
