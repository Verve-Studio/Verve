...
export function BevelOptions({
  layer,
  parentLayerName,
}: BevelOptionsProps): React.JSX.Element {
    ...
    <div className={styles.content}>
      ...
      <div className={styles.row}>
        ...
        <input
          ...
          onChange={(e) => {
            ...
            if (!isNaN(v))
              update({ width: Math.min(50, Math.max(1, Math.round(v))) });
          }}
        />
      ...
      <div className={styles.row}>
        ...
        <input
          ...
          onChange={(e) => {
            const v = e.target.valueAsNumber;
            if (!isNaN(v))
              update({ softness: Math.min(50, Math.max(0, Math.round(v))) });
          }}
        />
        <span className={styles.unitLabel}>px</span>
      </div>
      ...
      <div className={styles.row}>
        ...
        <input
          ...
          onChange={(e) => {
            const v = e.target.valueAsNumber;
            if (!isNaN(v))
              update({ angle: Math.min(360, Math.max(0, Math.round(v))) });
          }}
        />
      ...
      <div className={styles.footer}>
        ...
        <button
          ...
          onClick={() =>
            update({ width: 5, softness: 3, angle: 135, strength: 80 })
          }
        >
          Reset
        </button>
