 ...
 */
let _shapePreviewCache: { key: string; dataUrl: string; size: number } | null =
  null;
...
export function getPencilShapePreviewDataUrl(
  ...
  if (_shapePreviewCache?.key === cacheKey) {
    return {
      dataUrl: _shapePreviewCache.dataUrl,
      size: _shapePreviewCache.size,
    };
  }
...
function createPencilHandler(): ToolHandler {
  ...
  function draw1pxSegment(tx: number, ty: number, ctx: ToolContext): void {
    ...
    if (!ctx.tiledMode) {
      const W = ctx.renderer.pixelWidth,
        H = ctx.renderer.pixelHeight;
      const minX = Math.min(lastPx.x, x1),
        maxX = Math.max(lastPx.x, x1);
      const minY = Math.min(lastPx.y, y1),
        maxY = Math.max(lastPx.y, y1);
    ...
    if (pencilOptions.pixelPerfect) {
      ...
      bresenham(lastPx.x, lastPx.y, x1, y1, (px, py) => {
        if (first) {
          first = false;
          return;
        } // skip pixel[0] = lastPx, already painted
    ...
    if (!ctx.tiledMode) {
      ...
      const rx = Math.min(
        layer.layerWidth,
        Math.max(lastPx.x, x1) - layer.offsetX + 2,
      );
      const ry = Math.min(
        layer.layerHeight,
  ...
  return {
    ...
    onPointerMove({ x, y }: ToolPointerPos, ctx: ToolContext) {
      ...
      if (pencilOptions.size === 1 || brush) {
        ...
        if (
          Math.hypot(tipX - lastRendered.x, tipY - lastRendered.y) >= spacing
        ) {
          paint(
            lastRendered.x,
            lastRendered.y,
...
function PencilOptions({
  ...
  const handleSmoothing = (v: number): void => {
    pencilOptions.smoothing = v;
    setSmoothing(v);
  };
  const handleMotionBlur = (v: number): void => {
    pencilOptions.motionBlur = v;
    setMotionBlur(v);
  };
    ...
    <>
      ...
      {shades.map((shade, i) => (
        <button
          key={i}
          title={
            i === 2 ? "Current color" : i < 2 ? "Darker shade" : "Lighter shade"
          }
          style={{
            width: 14,
            height: 14,
      ...
      <label
        ...
        title="Remove L-corner pixels for clean pixel-art diagonals (1px only)"
      >
        <input
          type="checkbox"
      ...
      <label
        ...
        title="Filter pointer noise — higher values smooth the path at the cost of slight lag"
      >
        Smoothing:
      </label>
      <SliderInput
      ...
      <label
        className={styles.optLabel}
        title="Elongates dabs along the stroke direction for a calligraphic smear"
      >
        Motion:
      </label>
      <SliderInput
      ...
      {activeBrush && (
        <>
          <label
            className={styles.optCheckLabel}
            title="Snap to brush grid — stamps tile perfectly without overlap"
          >
            <input
              type="checkbox"
              checked={snapToBrush}
