...
export function useObjectSelection({
  ...
  const runInference = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      let upsampled = await upsampleMask(
        new Uint8Array(decodeResult.mask),
        width,
        height,
      );
      ...
      upsampled = keepComponentsContainingSeeds(
        upsampled,
        width,
        height,
        seeds,
    ...
    try {
      ...
      const erodeR =
        objectSelectionOptions.refineMode === "object"
          ...
          : Math.max(
              8,
              Math.min(48, Math.round(Math.min(cropW, cropH) * 0.05)),
            );
      const dilateR =
        objectSelectionOptions.refineMode === "object"
          ? Math.max(
              20,
              Math.min(60, Math.round(Math.min(cropW, cropH) * 0.04)),
            )
          : erodeR;
      ...
      if (objectSelectionOptions.refineMode === "object") {
        ...
        console.log(
          `[refineEdge] crop=${cropW}×${cropH} erode=${erodeR} dilate=${dilateR} trimap=${tTri.toFixed(0)}ms grabcut=${tGC.toFixed(0)}ms`,
        );
      } else {
  ...
  useEffect(() => {
    ...
    const onStoreChange = (): void => {
      ...
      if (store.promptMode === "rect") {
        const dragEnded =
          prevIsDragging && !store.isDragging && store.dragRect !== null;
