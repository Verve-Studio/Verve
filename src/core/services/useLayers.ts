...
export function useLayers({
  ...
  const handleMergeSelected = useCallback(
    async (ids: string[]): Promise<void> => {
      try {
        ...
        const selectedRoots = layers.filter(
          (l) => selectedSet.has(l.id) && isPixelRootLayer(l),
        );
        ...
        const merged = (await handle.rasterizeLayers(mergeLayers, "merge"))
          .data;
  ...
  const handleMergeVisible = useCallback(async (): Promise<void> => {
    try {
      ...
      const visibleRoots = layers.filter(
        (l) => l.visible && isPixelRootLayer(l),
      );
  ...
  const handleNewLayer = useCallback((): void => {
    ...
    dispatch({
      ...
      payload: {
        id,
        name: `Layer ${stateRef.current.layers.length + 1}`,
        visible: true,
        opacity: 1,
        locked: false,
        blendMode: "normal",
  ...
  const handleFlattenImage = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      if (!handle)
        throw new Error(
          "Canvas renderer is not ready yet. Please try flattening again.",
        );
