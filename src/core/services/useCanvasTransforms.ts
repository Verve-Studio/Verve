...
export function useCanvasTransforms({
      ...
      try {
        ...
        for (const layer of stateRef.current.layers) {
          if (isIndexed) {
            ...
            const resizedRgba = await resizeNearest(
              rgba,
              oldW,
              oldH,
              newW,
            ...
            const resizedIndices = await matchPaletteIndices(
              resizedRgba,
              swatches,
              255,
            );
  ...
  const handleResizeCanvas = useCallback(
    (settings: ResizeCanvasSettings): void => {
      ...
      const offsetX =
        anchorCol === 0
          ? 0
          : anchorCol === 1
            ? Math.round((newW - oldW) / 2)
            : newW - oldW;
      const offsetY =
        anchorRow === 0
          ? 0
          : anchorRow === 1
            ? Math.round((newH - oldH) / 2)
            : newH - oldH;
        ...
        if (isIndexed) {
          ...
          if (copyW > 0 && copyH > 0) {
            for (let row = 0; row < copyH; row++) {
              const srcOffset =
                (copyY + row - offsetY) * oldW + (copyX - offsetX);
              const dstOffset = (copyY + row) * newW + copyX;
              newIndices.set(
                indexData.subarray(srcOffset, srcOffset + copyW),
                dstOffset,
              );
            }
          }
          ...
          if (cpW > 0 && cpH > 0) {
            for (let row = 0; row < cpH; row++) {
              ...
              newPixels.set(
                oldPixels.subarray(srcOff, srcOff + cpW * 4),
                dstOff,
              );
            }
          }
  ...
  const handleCrop = useCallback((): void => {
    ...
    dispatch({
      type: "RESIZE_CANVAS",
      payload: { width: cropW, height: cropH },
    });
  }, [
    canvasWidth,
    canvasHeight,
    ...
    async (amount: RotateAmount): Promise<void> => {
      ...
      try {
        const entries = await Promise.all(
          layers.map(async (layer): Promise<[string, string] | null> => {
            if (isIndexed) {
  ...
  const handleRotateSelectedLayers = useCallback(
    async (amount: RotateAmount): Promise<void> => {
      const handle = canvasHandleRef.current;
      if (!handle) return;
      ...
      try {
          ...
          await Promise.all(
            layers
              ...
              .map(async (layer): Promise<Op | null> => {
                ...
                const lw = gpuLayer.layerWidth,
                  lh = gpuLayer.layerHeight;
                const raw = handle.getLayerRawData(layer.id);
                if (!raw) return null;
                if (gpuLayer.format === "indexed8") {
                  const result = await rotateIndexed(
                    raw as Uint8Array,
                    lw,
                    lh,
                    wasmAmount,
                  ...
                  return {
                    id: layer.id,
                    data: result,
                    newW: amount === "180" ? lw : lh,
                    newH: amount === "180" ? lh : lw,
                    fmt: "indexed8",
                  };
                } else if (gpuLayer.format === "rgba32f") {
                  const { data, dstW, dstH } = rotateF32(
                    raw as Float32Array,
                    lw,
                    lh,
                    wasmAmount,
        ...
        for (const op of ops) {
          ...
          handle.replaceLayerData(
            ...
            op.fmt,
            op.fmt === "indexed8"
              ? (swatches as import("@/types").RGBAColor[])
              : undefined,
          );
        }
      } catch (err) {
          ...
          await Promise.all(
            layers
              ...
              .map(async (layer): Promise<Op | null> => {
                ...
                if (!raw) return null;
                if (gpuLayer.format === "indexed8") {
                  const result = await flipIndexed(
                    raw as Uint8Array,
                    lw,
                    lh,
                    wasmAxis,
                  ...
                  return {
                    id: layer.id,
                    data: flipF32(raw as Float32Array, lw, lh, wasmAxis),
                    fmt: "rgba32f",
                  };
                } else {
                  const result = await flipRgba(
                    raw as Uint8Array,
                    lw,
                    lh,
                    wasmAxis,
