...
export type AlignEdge = 'left' | 'centerV' | 'right' | 'top' | 'centerH' | 'bottom'
...
function computeCanvasBbox(layer: GpuLayer): { l: number; t: number; r: number; b: number } | null {
  const { data, format, layerWidth: lw, layerHeight: lh, offsetX: ox, offsetY: oy } = layer
...
export function useLayerArrange({
  ...
  const handleAlign = useCallback((edge: AlignEdge): void => {
    const handle = canvasHandleRef.current
    if (!handle) return
    const { layers } = stateRef.current
    const ids = getEffectiveIds()

    type LayerInfo = { gpuLayer: GpuLayer; bbox: NonNullable<ReturnType<typeof computeCanvasBbox>> }
    ...
    switch (edge) {
      case 'left':    anchorX = Math.min(...infos.map(i => i.bbox.l)); break
      case 'right':   anchorX = Math.max(...infos.map(i => i.bbox.r)); break
      case 'centerV': anchorX = Math.round((Math.min(...infos.map(i => i.bbox.l)) + Math.max(...infos.map(i => i.bbox.r))) / 2); break
      case 'top':     anchorY = Math.min(...infos.map(i => i.bbox.t)); break
      case 'bottom':  anchorY = Math.max(...infos.map(i => i.bbox.b)); break
      case 'centerH': anchorY = Math.round((Math.min(...infos.map(i => i.bbox.t)) + Math.max(...infos.map(i => i.bbox.b))) / 2); break
    }

    captureHistory('Before Align')
    for (const { gpuLayer, bbox } of infos) {
      switch (edge) {
        case 'left':    gpuLayer.offsetX += anchorX - bbox.l; break
        case 'right':   gpuLayer.offsetX += anchorX - bbox.r; break
        case 'centerV': gpuLayer.offsetX += anchorX - Math.round((bbox.l + bbox.r) / 2); break
        case 'top':     gpuLayer.offsetY += anchorY - bbox.t; break
        case 'bottom':  gpuLayer.offsetY += anchorY - bbox.b; break
        case 'centerH': gpuLayer.offsetY += anchorY - Math.round((bbox.t + bbox.b) / 2); break
      }
    }
    handle.invalidate()
  ...
  const handleDistribute = useCallback((axis: DistributeAxis): void => {
    const handle = canvasHandleRef.current
    if (!handle) return
    const { layers } = stateRef.current
    const ids = getEffectiveIds()
    ...
    if (axis === 'horizontal') {
      ...
      const lastCenter  = (infos[infos.length - 1].bbox.l + infos[infos.length - 1].bbox.r) / 2
      ...
      const lastCenter  = (infos[infos.length - 1].bbox.t + infos[infos.length - 1].bbox.b) / 2
      const step = (lastCenter - firstCenter) / (infos.length - 1)
      for (let i = 1; i < infos.length - 1; i++) {
        const { gpuLayer, bbox } = infos[i]
        const targetCenter = firstCenter + i * step
  ...
  const handleOrder = useCallback((op: OrderOp): void => {
    const { layers } = stateRef.current
    const ids = getEffectiveIds()
    ...
    const isSel = (c: (typeof clusters)[number]): boolean => selectedRootIds.has(c[0].id)

    let newClusters: typeof clusters

    switch (op) {
      case 'front': {
        newClusters = [...clusters.filter(c => !isSel(c)), ...clusters.filter(isSel)]
        break
      }
      case 'back': {
        newClusters = [...clusters.filter(isSel), ...clusters.filter(c => !isSel(c))]
        break
      }
      case 'forward': {
        ...
        for (let i = newClusters.length - 2; i >= 0; i--) {
          if (isSel(newClusters[i]) && !isSel(newClusters[i + 1])) {
            ;[newClusters[i], newClusters[i + 1]] = [newClusters[i + 1], newClusters[i]]
          }
        }
        break
      }
      ...
      case 'reverse': {
        newClusters = [...clusters]
        const selIdxs = newClusters.map((c, i) => (isSel(c) ? i : -1)).filter(i => i >= 0)
    ...
    dispatch({ type: 'REORDER_LAYERS', payload: [...newClusters.flat(), ...remaining] })
