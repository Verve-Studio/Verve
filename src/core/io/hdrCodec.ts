...
function decodeScanline(data: Uint8Array, offset: number, width: number, scanline: Uint8Array): number {
  ...
  for (let ch = 0; ch < 4; ch++) {
    ...
    while (idx < width) {
      ...
      if (code > 128) {
        ...
        // Non-run
        for (let i = 0; i < code; i++) scanline[ch + idx++ * 4] = data[offset++]
      }
    }
  }
...
export function decodeRgbe(bytes: Uint8Array): { pixels: Float32Array; width: number; height: number } {
  ...
  if (!header.includes('FORMAT=32-bit_rle_rgbe')) throw new Error('HDR: unsupported format')
  offset = headerEnd
...
function floatToRgbe(r: number, g: number, b: number): [number, number, number, number] {
  ...
  return [Math.min(255, Math.round(r * scale)), Math.min(255, Math.round(g * scale)), Math.min(255, Math.round(b * scale)), exp + 128]
}
...
function encodeRleScanline(channel: Uint8Array): Uint8Array {
  ...
  while (i < channel.length) {
    // Find run
    let runLen = 1
    while (runLen < 127 && i + runLen < channel.length && channel[i + runLen] === channel[i]) runLen++
    if (runLen > 2) {
      ...
      while (nonRunLen < 128 && i + nonRunLen < channel.length) {
        let nextRun = 1
        while (nextRun < 3 && i + nonRunLen + nextRun < channel.length && channel[i + nonRunLen + nextRun - 1] === channel[i + nonRunLen]) nextRun++
        if (nextRun >= 3) break
        nonRunLen++
      }
      out.push(nonRunLen)
...
export function encodeRgbe(pixels: Float32Array, width: number, height: number): Uint8Array {
  ...
  for (let y = 0; y < height; y++) {
    ...
    const scanHeader = new Uint8Array([2, 2, (width >> 8) & 0xff, width & 0xff])
    ...
    for (let x = 0; x < width; x++) {
      const pi = (y * width + x) * 4
      const [r, g, b, e] = floatToRgbe(pixels[pi], pixels[pi + 1], pixels[pi + 2])
      channels[0][x] = r
      channels[1][x] = g
      channels[2][x] = b
      channels[3][x] = e
