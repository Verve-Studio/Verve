...
export function exportTiff32(
  ...
  writeTag(view, off, TAG_IMAGE_WIDTH, TYPE_LONG, 1, width);
  off += 12;
  writeTag(view, off, TAG_IMAGE_LENGTH, TYPE_LONG, 1, height);
  off += 12;
  writeTag(view, off, TAG_BITS_PER_SAMPLE, TYPE_SHORT, 4, bpsOffset);
  off += 12;
  writeTag(view, off, TAG_COMPRESSION, TYPE_SHORT, 1, 1 /* none */);
  off += 12;
  writeTag(view, off, TAG_PHOTO_INTERP, TYPE_SHORT, 1, 2 /* RGB */);
  off += 12;
  writeTag(view, off, TAG_STRIP_OFFSETS, TYPE_LONG, 1, imageDataOffset);
  off += 12;
  writeTag(view, off, TAG_SAMPLES_PER_PIXEL, TYPE_SHORT, 1, samplesPerPixel);
  off += 12;
  writeTag(view, off, TAG_ROWS_PER_STRIP, TYPE_LONG, 1, height);
  off += 12;
  writeTag(view, off, TAG_STRIP_BYTE_COUNTS, TYPE_LONG, 1, imageDataSize);
  off += 12;
  writeTag(view, off, TAG_PLANAR_CONFIG, TYPE_SHORT, 1, 1 /* chunky */);
  off += 12;
  writeTag(view, off, TAG_SAMPLE_FORMAT, TYPE_SHORT, 4, sfOffset);
  off += 12;
  // ExtraSamples (tag 338): 1 sample = 2 (unassoc alpha)
  writeTag(view, off, 338, TYPE_SHORT, 1, 2);
  off += 12;
  // YResolution (tag 283): ignored, store a dummy (1/1) — skip for minimal compliance
  // Use a padding tag that readers ignore
  writeTag(view, off, TAG_IMAGE_WIDTH, TYPE_LONG, 1, width);
  off += 12; // duplicated fine per TIFF spec (readers take first)

  // Next IFD = 0
  writeUint32(view, off, 0);
  off += 4;

  // BitsPerSample values: 32, 32, 32, 32
  for (let i = 0; i < 4; i++) {
    writeUint16(view, bpsOffset + i * 2, 32);
  }

  // SampleFormat values: 3, 3, 3, 3 (IEEE float)
  for (let i = 0; i < 4; i++) {
    writeUint16(view, sfOffset + i * 2, 3);
  }
