...
export function InnerShadowOptions({ layer, parentLayerName }: InnerShadowOptionsProps): React.JSX.Element {
    ...
    <div className={styles.content}>
      ...
      <div className={styles.row}>
        ...
        <div className={styles.signedTrackWrap}>
          <input
            ...
            value={p.offsetX}
            style={{ '--pct': pct(p.offsetX, -200, 200) } as React.CSSProperties}
        ...
        <input
          ...
          onChange={(e) => {
            const v = e.target.valueAsNumber
            if (!isNaN(v)) update({ offsetX: Math.min(200, Math.max(-200, Math.round(v))) })
          }}
        />
      ...
      <div className={styles.row}>
        ...
        <div className={styles.signedTrackWrap}>
          <input
            ...
            min={-200} max={200} step={1}
            value={p.offsetY}
            style={{ '--pct': pct(p.offsetY, -200, 200) } as React.CSSProperties}
        ...
        <input
          ...
          onChange={(e) => {
            const v = e.target.valueAsNumber
            if (!isNaN(v)) update({ offsetY: Math.min(200, Math.max(-200, Math.round(v))) })
          }}
        />
        <span className={styles.unitLabel}>px</span>
      </div>
      ...
      <div className={styles.footer}>
        ...
        <button
          ...
          onClick={() => update({ color: { r: 0, g: 0, b: 0, a: 255 }, opacity: 75, offsetX: 5, offsetY: 5, spread: 0, softness: 10 })}
        >
          Reset
        </button>
      </div>
