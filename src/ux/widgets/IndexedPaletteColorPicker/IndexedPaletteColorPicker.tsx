...
export function IndexedPaletteColorPicker({
  ...
  const [clampedPos, setClampedPos] = useState<{ x: number; y: number } | null>(anchorPos ?? null)
      ...
      <div className={styles.grid}>
        {palette.map((color, idx) => {
          ...
          return (
            <div
              key={idx}
              className={[styles.cell, idx === activeIndex ? styles.active : ''].join(' ')}
              style={{ background: bg }}
              title={`idx ${idx}  ${toHex(color)}`}
              onPointerEnter={() => setHovered(idx)}
              onPointerLeave={() => setHovered(null)}
              onClick={() => { onSelect(idx, color); onClose() }}
            />
          )
        })}
      </div>
        ...
        {footerColor !== null && footerIdx !== null ? (
          <>
            <div
              ...
              style={{ background: `rgba(${footerColor.r},${footerColor.g},${footerColor.b},${footerColor.a / 255})` }}
            />
