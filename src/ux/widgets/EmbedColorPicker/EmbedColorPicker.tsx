...
export function EmbedColorPicker({
  ...
  grayscaleOnly = false,
  pixelFormat = "rgba8",
}: EmbedColorPickerProps): React.JSX.Element {
  ...
  const lastEmittedRef = useRef<{ r: number; g: number; b: number } | null>(
    null,
  );
  useEffect(() => {
    ...
    if (
      last &&
      Math.abs(last.r - value.r) < 1e-9 &&
      Math.abs(last.g - value.g) < 1e-9 &&
      Math.abs(last.b - value.b) < 1e-9
    )
      return;
  ...
  const fire = useCallback(
    (r: number, g: number, b: number, currentIntensity: number): void => {
      setRgb([r, g, b]);
      const emitted = {
        r: r * currentIntensity,
        g: g * currentIntensity,
        b: b * currentIntensity,
      };
  ...
  const onRgbChannelChange = (ch: number, n: number): void => {
    ...
    const c =
      pixelFormat === "rgba32f" ? clamp(n, 0, 1) : clamp(n, 0, 255) / 255;
            ...
            return (
              <div key={ch} className={styles.channelRow}>
                ...
                <input
                  ...
                  onChange={(e) =>
                    onRgbChannelChange(i, parseFloat(e.target.value))
                  }
                />
                <SliderInput
                  min={displayMin}
          ...
          {[
            ...
            <div key={label} className={styles.channelRow}>
              ...
              <input
                ...
                onChange={(e) =>
                  onHsvChannelChange(ch, parseInt(e.target.value))
                }
              />
              <SliderInput
                min={min}
