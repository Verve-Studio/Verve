...
export function rasterizeTextToLayer(ls: TextLayerState, gl: GpuLayer): void {
  ...
  ;(ctx2d as CanvasRenderingContext2D & { letterSpacing?: string }).letterSpacing = `${ls.letterSpacing ?? 0}px`
  if ('fontKerning' in ctx2d) {
    (ctx2d as CanvasRenderingContext2D & { fontKerning?: string }).fontKerning = (ls.kerning ?? 'auto') === 'auto' ? 'auto' : 'none'
  }
