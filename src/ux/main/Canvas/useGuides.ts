...
export function useGuides({
  dispatch, showRulers, showGuides, zoom,
  hRulerRef, vRulerRef, canvasWrapperRef,
}: UseGuidesParams): {
  dragPreview: GuideDragPreview | null
  startGuideDrag: (e: React.PointerEvent, guideId: string, axis: 'h' | 'v') => void
} {
  ...
  const dragRef = useRef<{ axis: 'h' | 'v'; guideId: string | null } | null>(null)
  ...
  const startDrag = useCallback((axis: 'h' | 'v', guideId: string | null) => {
    ...
    const onUp = (e: PointerEvent) => {
      ...
      if (!drag) return
      const hit = hitTest(e.clientX, e.clientY)
      const position = Math.round(drag.axis === 'h' ? (hit?.docY ?? 0) : (hit?.docX ?? 0))

      if (drag.guideId === null) {
        // New guide — only commit if dropped inside canvas
        if (hit?.inside) {
          dispatchRef.current({ type: 'ADD_GUIDE', payload: { id: `g${Date.now()}`, axis: drag.axis, position } })
        }
      } else {
        // Existing guide — delete if dragged off canvas, else move
        if (!hit?.inside) {
          dispatchRef.current({ type: 'DELETE_GUIDE', payload: drag.guideId })
        } else {
          dispatchRef.current({ type: 'MOVE_GUIDE', payload: { id: drag.guideId, position } })
        }
      }
    }
  ...
  const startGuideDrag = useCallback((e: React.PointerEvent, guideId: string, axis: 'h' | 'v') => {
    if (e.button !== 0) return
    e.stopPropagation()
