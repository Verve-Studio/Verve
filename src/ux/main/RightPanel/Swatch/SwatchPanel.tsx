...
export function SwatchPanel({
  ...
  const [pendingSwatchRemoval, setPendingSwatchRemoval] = useState<
    number | null
  >(null);
  ...
  const [groupPromptMode, setGroupPromptMode] = useState<"create" | "rename">(
    "create",
  );
  ...
  if (
    activeGroupId !== null &&
    !state.swatchGroups.some((g) => g.id === activeGroupId)
  ) {
    setActiveGroupId(null);
  }
  ...
  const handleSwatchClick = useCallback(
    (e: React.MouseEvent, canonicalIndex: number, displayIndex: number) => {
      if (e.shiftKey && anchorIndexRef.current !== null) {
        ...
        setSelectedIndices(
          displayEntries.slice(lo, hi + 1).map((entry) => entry.canonicalIndex),
        );
        return;
      }
      ...
      dispatch({
        ...
        payload: { r: sw.r / 255, g: sw.g / 255, b: sw.b / 255, a: sw.a / 255 },
      });
  ...
  const handleContextMenu = useCallback(
    (e: React.MouseEvent, canonicalIndex: number) => {
      e.preventDefault();
  ...
  function handleGroupPromptConfirm() {
    ...
    if (groupPromptMode === "rename") {
      const conflict = state.swatchGroups.find(
        (g) => g.name === name && g.id !== activeGroupId,
      );
      if (conflict) {
      ...
      dispatch({
        type: "RENAME_SWATCH_GROUP",
        payload: { id: activeGroupId!, name },
      });
    } else {
      dispatch({
        type: "ADD_SWATCH_GROUP",
        payload: { name, swatchIndices: groupPromptIndicesRef.current },
      });
    }
    ...
    >
      <div className={styles.actions}>
        <select
          ...
          title={
            activeGroupId !== null
              ? (state.swatchGroups.find((g) => g.id === activeGroupId)?.name ??
                "")
              : "All swatches"
          }
        >
            ...
            ReactDOM.createPortal(
              ...
              >
                ...
                <button
                  ...
                  onClick={() => {
                    setMenuOpen(false);
                    dispatch({ type: "SET_SWATCHES", payload: [] });
                  }}
                >
                  Clear Palette
                </button>
      ...
      <div className={styles.swatchGrid}>
        {displayEntries.map((entry, displayIndex) => {
          ...
          const isGroupHighlighted =
            highlightedCanonicalIndices.has(canonicalIndex);
          const cellClass = [
            styles.swatchCell,
          ...
          return (
            <button
              ...
              onClick={(e) =>
                handleSwatchClick(e, canonicalIndex, displayIndex)
              }
        ...
        {state.swatches.length === 0 && (
          <span className={styles.swatchesEmpty}>
            No swatches yet. Add colors from the Color Picker.
          </span>
        )}
      </div>
        ...
        ReactDOM.createPortal(
          ...
          >
            <button
              ...
              onClick={() => {
                if (
                  state.pixelFormat === "indexed8" &&
                  state.swatches.length <= 1
                ) {
                  showOperationError(
                ...
                dispatch({
                  type: "REMOVE_SWATCH",
                  payload: contextMenu.canonicalIndex,
                });
                setContextMenu(null);
              }}
            >
              ...
              <button
                ...
                onClick={() => {
                  ...
                  dispatch({
                    ...
                    payload: { id: activeGroupId, swatchIndices: indices },
                  });
                  setContextMenu(null);
                }}
              >
            ...
            {activeGroupId !== null &&
              activeGroupMemberSet.has(contextMenu.canonicalIndex) && (
                <button
                  type="button"
                  className={styles.contextMenuItem}
      ...
      <ModalDialog
        title={
          groupPromptMode === "create" ? "Create / Join Group" : "Rename Group"
        }
      ...
      >
        <div className={styles.promptBody}>
          <input
            ...
            onChange={(e) => {
              setGroupPromptName(e.target.value);
              setGroupPromptError(null);
            }}
            onKeyDown={(e) => {
              if (e.key === "Enter") handleGroupPromptConfirm();
            }}
            autoFocus
            placeholder="Group name"
          />
          {groupPromptError && (
            <p className={styles.promptError}>{groupPromptError}</p>
          )}
          <div className={styles.promptButtons}>
            <DialogButton primary onClick={handleGroupPromptConfirm}>
              OK
            </DialogButton>
            <DialogButton
              onClick={() => {
      ...
      >
        <div className={styles.promptBody}>
          ...
          >
            Removing this swatch will shift palette indices in all pixel layers.
          ...
          <div className={styles.promptButtons}>
            ...
            <DialogButton onClick={() => setPendingSwatchRemoval(null)}>
              Cancel
            </DialogButton>
          </div>
        </div>
