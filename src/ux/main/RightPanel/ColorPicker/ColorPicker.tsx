...
import {
  EmbedColorPicker,
  toHex,
} from "@/ux/widgets/EmbedColorPicker/EmbedColorPicker";
...
export function ColorPicker(): React.JSX.Element {
  ...
  const grayscaleOnly = !!(
    activeLayerData &&
    "type" in activeLayerData &&
    activeLayerData.type === "mask"
  );

  const onPrimaryChange = (c: RGBAColor): void => {
    dispatch({ type: "SET_PRIMARY_COLOR", payload: c });
  };
  const onSecondaryChange = (c: RGBAColor): void => {
    dispatch({ type: "SET_SECONDARY_COLOR", payload: c });
  };
  ...
  const bgHex = toHex(
    Math.round(Math.min(secondaryColor.r, 1) * 255),
    Math.round(Math.min(secondaryColor.g, 1) * 255),
    Math.round(Math.min(secondaryColor.b, 1) * 255),
  );
    ...
    <div className={styles.picker}>
      ...
      <div className={styles.swatchRow}>
        ...
        <span className={styles.swatchLabel}>
          {active === "fg" ? "Foreground" : "Background"}
        </span>
      </div>
