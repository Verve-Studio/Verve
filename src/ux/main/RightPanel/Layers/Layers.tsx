...
const EyeIcon = ({ visible }: { visible: boolean }): React.JSX.Element =>
  visible ? (
    ...
    >
      <path
        d="M2 2l10 10M5.5 4.2A5.5 5.5 0 017 4c3 0 5 2.5 5.5 3-.4.7-1.3 1.8-2.5 2.5M3 5.5C2 6.2 1.5 6.8 1.5 7c.5.5 2.5 3 5.5 3 .6 0 1.2-.1 1.7-.3"
        strokeLinecap="round"
      />
    </svg>
  );
...
function highlightMatch(name: string, query: string): React.ReactNode {
  ...
  return (
    <>
      {name.slice(0, idx)}
      <mark className={styles.nameMatchMark}>
...
export function Layers({
  ...
  const onActiveLayerChange = (id: string): void => {
  ...
  const onLayerToggleVisibility = (id: string): void => {
    dispatch({ type: "TOGGLE_LAYER_VISIBILITY", payload: id });
  };
  const onLayerToggleLock = (id: string): void => {
    dispatch({ type: "TOGGLE_LAYER_LOCK", payload: id });
  };
  const onLayerOpacityChange = (id: string, opacity: number): void => {
  ...
  const setSelectedIds = (next: Set<string>): void => {
    dispatch({ type: "SET_SELECTED_LAYERS", payload: [...next] });
  };
  ...
  const [adjDropTarget, setAdjDropTarget] = useState<
  ...
  const [collapsedPixelLayers, setCollapsedPixelLayers] = useState<Set<string>>(
    new Set(),
  );
  const togglePixelLayerCollapse = (id: string): void =>
    setCollapsedPixelLayers((prev) => {
  ...
  const filteredRows: TreeRow[] = useMemo((): TreeRow[] => {
    ...
    function markAllDescendants(
      group: GroupLayerState | CompositeLayerState,
    ): void {
    ...
    function subtreeHasMatch(
      ...
      for (const childId of group.childIds) {
        ...
        const child = layerMap.get(childId);
        if (child && isContainerLayer(child) && subtreeHasMatch(child))
          return true;
      }
      if (isCompositeLayer(group)) {
        for (const l of layers) {
  ...
  const displayActiveId = useMemo((): string | null => {
    ...
    if (treeRows.some((r) => r.layer.id === activeLayerId))
  ...
  const handleLayerClick = (layer: LayerState, e: React.MouseEvent): void => {
    ...
    if (isMultiKey) {
      ...
      const anchorIdx = filteredRows.findIndex(
        (r) => r.layer.id === anchorLayerIdRef.current,
      );
      ...
      if (anchorIdx >= 0 && clickIdx >= 0) {
        ...
        setSelectedIds(
          new Set(filteredRows.slice(lo, hi + 1).map((r) => r.layer.id)),
        );
      }
    } else {
  ...
  const effectiveDeleteIds = [
    ...new Set([...selectedIds, ...(activeLayerId ? [activeLayerId] : [])]),
  ];
  ...
  const isActiveComposite =
    activeLayer !== undefined && isCompositeLayer(activeLayer);
      ...
      (!("type" in activeLayer) &&
        layers.some(
          (l) =>
            "type" in l &&
            l.type === "adjustment" &&
            (l as { parentId: string }).parentId === activeLayerId,
        )));
  ...
  const opacityValue =
    !isChildActive && !isActiveGroup && activeLayer
      ? Math.round((activeLayer as { opacity: number }).opacity * 100)
      : 100;
  const blendValue: BlendMode =
    !isChildActive && !isActiveGroup && activeLayer
      ? (activeLayer as { blendMode: BlendMode }).blendMode
      : "normal";

  const activeBlendModes = isActiveComposite
    ? BLEND_MODES_GROUP
    : BLEND_MODES_BASE;
  ...
  const handleAdjDrop = (e: React.DragEvent, parentId: string): void => {
    ...
    const targetId =
      "beforeId" in adjDropTarget
        ? adjDropTarget.beforeId
        : adjDropTarget.afterId;
    ...
    dispatch({
      ...
      payload: { parentId, orderedChildIds: without },
    });
  };
  ...
  const handleDragOver = (
    ...
    if (isDescendantOf(layers, layerId, srcId)) {
      e.dataTransfer.dropEffect = "none";
      return;
    }
    e.dataTransfer.dropEffect = "move";
  ...
  const handleDrop = (e: React.DragEvent): void => {
    ...
    const dragSet = effectiveSelected.has(srcId)
      ? effectiveSelected
      : new Set([srcId]);
    ...
    if (dropTarget.kind === "into") {
      ...
      const validIds = orderedDragIds.filter(
        (id) => id !== gId && !isDescendantOf(layers, gId, id),
      );
      ...
      validIds.forEach((id, i) => {
        dispatch({
          ...
          payload: { layerId: id, targetGroupId: gId, insertIndex: i },
        });
      });
      return;
    }
    ...
    const validIds = orderedDragIds.filter(
      (id) => id !== targetId && !isDescendantOf(layers, targetId, id),
    );
    if (validIds.length === 0) return;
    ...
    if (targetParent) {
      ...
      const baseIndex =
        dropTarget.kind === "before" ? targetIdx : targetIdx + 1;
      validIds.forEach((id, i) => {
        dispatch({
          type: "MOVE_LAYER_INTO_GROUP",
          payload: {
            layerId: id,
            targetGroupId: targetParent.id,
            insertIndex: baseIndex + i,
          },
        });
      });
    } else {
        ...
        if (validIds.length === 1) {
          ...
          const dstDisplayIdx =
            dropTarget.kind === "before"
              ? targetDisplayIdx
              : targetDisplayIdx + 1;
          const newLayers = reorderRootLayers(
            layers,
            validIds[0],
            dstDisplayIdx,
          );
  ...
  return (
    <div
      className={[
        styles.panel,
        filterQuery !== "" ? styles.panelFiltered : "",
      ].join(" ")}
    >
      {/* ── Blend mode + Opacity ──────────────────────────────────────── */}
      ...
      <div className={styles.blendRow}>
        <select
          ...
          onChange={(e) =>
            activeLayer &&
            onLayerBlendChange(activeLayer.id, e.target.value as BlendMode)
          }
        >
        ...
        <SliderInput
          ...
          disabled={!activeLayer || !!isChildActive || isActiveGroup}
          onChange={(n) =>
            activeLayer && onLayerOpacityChange(activeLayer.id, n / 100)
          }
        />
      </div>
        ...
        <div className={styles.lockRow}>
          ...
          <button
            ...
            title={
              activeLayer &&
              !isChildActive &&
              (activeLayer as { locked?: boolean }).locked
                ? "Unlock layer"
                : "Lock layer"
            }
            disabled={!activeLayer || !!isChildActive}
            onClick={() =>
              activeLayer && !isChildActive && onLayerToggleLock(activeLayer.id)
            }
          >
            <LockIcon
              locked={
                (!isChildActive &&
                  (activeLayer as unknown as { locked?: boolean })?.locked) ??
                false
              }
            />
          </button>
        ...
        {filterQuery !== "" && filteredRows.length === 0 ? (
          <li className={styles.filterEmpty}>
            <span className={styles.filterEmptyText}>
              No layers match <em>&ldquo;{filterQuery}&rdquo;</em>
            </span>
          </li>
        ) : (
          filteredRows.map(({ layer, depth, hasChildren, isLastChild }) => {
            ...
            const isDropBefore =
              dropTarget?.kind === "before" && dropTarget.layerId === layer.id;
            const isDropAfter =
              dropTarget?.kind === "after" && dropTarget.layerId === layer.id;
            const isDropInto =
              dropTarget?.kind === "into" && dropTarget.groupId === layer.id;

            // Adjustment/mask child reorder indicators
            const isAdjDropBefore =
              isChild &&
              adjDropTarget &&
              "beforeId" in adjDropTarget &&
              adjDropTarget.beforeId === layer.id;
            const isAdjDropAfter =
              isChild &&
              adjDropTarget &&
              "afterId" in adjDropTarget &&
              adjDropTarget.afterId === layer.id;

            const isPixelCollapsed =
              !isGroup &&
              !isChild &&
              hasChildren &&
              collapsedPixelLayers.has(layer.id);
            ...
            return (
              <li
                ...
                className={[
                  ...
                  isDropBefore || isAdjDropBefore
                    ? styles.dropIndicatorBefore
                    : "",
                  isDropAfter || isAdjDropAfter
                ...
                onDragStart={(e) =>
                  isChild
                    ? handleAdjDragStart(layer.id, e)
                    : handleDragStart(layer.id, e)
                }
                onDragEnd={isChild ? handleAdjDragEnd : handleDragEnd}
                onDragOver={(e) =>
                  isChild && adjParentId
                    ? handleAdjDragOver(e, layer.id)
                    : !isChild && handleDragOver(e, layer.id, isGroup)
                }
                onDrop={(e) =>
                  isChild && adjParentId
                    ? handleAdjDrop(e, adjParentId)
                    : undefined
                }
              ...
              >
                ...
                {isGroup ? (
                  ...
                  <button
                    ...
                    aria-label={
                      isPixelCollapsed ? "Show adjustments" : "Hide adjustments"
                    }
                    title={
                      isPixelCollapsed ? "Show adjustments" : "Hide adjustments"
                    }
                ...
                {isAdjustment ? (
                  <div className={styles.adjThumb} aria-hidden="true">
                    <AdjustmentIcon />
                  </div>
                ) : isGroup ? (
                  ...
                  <div className={styles.textThumb} aria-hidden="true">
                    T
                  </div>
                ) : (
                  <div
                    className={`${styles.thumb} ${isMask ? styles.maskThumb : ""}`}
                    aria-hidden="true"
                  />
                )}
      ...
      <div className={styles.footer}>
        {filterQuery !== "" && (
          <span className={styles.filterCount}>
            {filteredRows.length} of {layers.length}
          </span>
        )}
        <button
        ...
        <button
          className={styles.footerBtn}
          onClick={onCreateCompositeLayer}
          aria-label="New composite layer"
          title="New composite layer"
        >
          <CompositeIcon />
        </button>
        <button
        ...
        <>
          ...
          >
            <button
              ...
              onMouseDown={() => {
                closeContextMenu();
                if (activeLayerId) onAddMaskLayer(activeLayerId);
              }}
            >
              Add Layer Mask
            </button>
            ...
            <button
              ...
              onMouseDown={() => {
                closeContextMenu();
                if (activeLayerId) onRasterizeLayer(activeLayerId);
              }}
            >
              Rasterize Layer
            </button>
            ...
            {isActiveGroup && (
              <button
                ...
                onMouseDown={() => {
                  closeContextMenu();
                  if (activeLayerId) onUngroup(activeLayerId);
                }}
              >
                Ungroup
              </button>
            ...
            {isActiveGroup && (
              <button
                ...
                onMouseDown={() => {
                  closeContextMenu();
                  if (activeLayerId) onMergeGroup(activeLayerId);
                }}
              >
                Merge Group
              </button>
            ...
            {(new Set([
              ...
              isActiveGroup) && <div className={styles.menuDivider} />}
            <button
              className={styles.menuItem}
              disabled={
                new Set([
