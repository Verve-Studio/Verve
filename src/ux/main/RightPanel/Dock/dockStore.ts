...
function removeFromRow(layout: DockLayout, panelId: PanelId, sourceRowId: string): DockLayout {
  return {
    ...layout,
    rows: layout.rows
      .map(r => {
        ...
        const activePanel = panels.includes(r.activePanel) ? r.activePanel : (panels[0] ?? r.activePanel)
...
function findRowContaining(layout: DockLayout, panelId: PanelId): DockRowConfig | undefined {
...
class DockStore {
  ...
  private _migrate(raw: Partial<DockLayout>): DockLayout {
    ...
    const rows: DockRowConfig[] = (raw.rows ?? [])
      .map(r => {
        const panels = (r.panels ?? []).filter((p): p is PanelId => known.has(p))
  ...
  moveToRow(panelId: PanelId, sourceRowId: string, targetRowId: string, insertAt: number): void {
  ...
  splitToNewRow(panelId: PanelId, sourceRowId: string, afterRowId: string | null): void {
    let next = removeFromRow(this._layout, panelId, sourceRowId)
    ...
    if (afterRowId === null) {
      ...
      next = {
        ...next,
        rows: [...next.rows.slice(0, insertIdx), newRow, ...next.rows.slice(insertIdx)],
      }
    }
    this._set(next)
  }
  ...
  openPanel(panelId: PanelId): void {
    ...
    if (rows.length === 0) {
      this._set({
        ...this._layout,
        rows: [{ id: `row-${Date.now()}`, panels: [panelId], activePanel: panelId, height: null }],
    ...
    this._set({
      ...
      rows: rows.map(r =>
        r.id === lastRow.id
          ? { ...r, panels: [...r.panels.filter(p => p !== panelId), panelId], activePanel: panelId }
          : r
      ),
      closedPanels: this._layout.closedPanels.filter(p => p !== panelId),
    })
  ...
  setRowHeight(rowId: string, height: number): void {
    this._set({
      ...this._layout,
      rows: this._layout.rows.map(r => r.id === rowId ? { ...r, height } : r),
    })
  }
