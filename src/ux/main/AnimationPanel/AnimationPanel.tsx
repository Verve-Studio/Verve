...
export function AnimationPanel({
  ...
  const cols =
    ss.cellWidth > 0 && canvasW > 0
      ? Math.max(1, Math.floor(canvasW / ss.cellWidth))
      : 1;

  const cellW = Math.max(1, ss.cellWidth);
  const cellH = Math.max(1, ss.cellHeight);
  const maxCells =
    Math.floor(Math.max(1, canvasW) / cellW) *
    Math.floor(Math.max(1, canvasH) / cellH);
  const totalFrames = ss.animations.reduce(
    (acc, a) => acc + a.frames.length,
    0,
  );
  const atCapacity = totalFrames >= maxCells;

  function sourceRectPx(frameIndex: number): {
    x: number;
    y: number;
    w: number;
    h: number;
  ...
  function sourceRectUV(px: { x: number; y: number; w: number; h: number }): {
    u: number;
    v: number;
    uw: number;
    vh: number;
  ...
  const onHandlePointerMove = (e: React.PointerEvent): void => {
    ...
    setWidth(
      Math.min(MAX_WIDTH, Math.max(MIN_WIDTH, dragStartW.current + delta)),
    );
  };
        ...
        <div className={styles.section}>
          ...
          <div className={styles.row}>
            ...
            <Toggle
              checked={ss.enabled}
              onChange={(v) =>
                dispatch({ type: "SET_SPRITESHEET", payload: { enabled: v } })
              }
            />
          </div>
          ...
          <div className={styles.section}>
            ...
            {ss.animations.map((anim) => (
              <div
                ...
                onClick={() =>
                  dispatch({ type: "SET_SELECTED_ANIMATION", payload: anim.id })
                }
              >
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      dispatch({
                        ...
                        payload: { ...anim, name: e.target.value },
                      })
                    }
                  />
                  <button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      dispatch({ type: "DELETE_ANIMATION", payload: anim.id });
                    }}
                  >
                    ×
                  </button>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      dispatch({
                        ...
                        payload: {
                          ...anim,
                          playbackMode: e.target.value as AnimationPlaybackMode,
                        },
                      })
                    }
                  >
              ...
              return (
                ...
                >
                  <div className={styles.frameHeader}>
                    ...
                    >
                      <button
                        ...
                        disabled={!hasPrev}
                        onClick={() =>
                          onCopyPrevFrame(selectedAnim.id, frame.id)
                        }
                      >
                        ← Prev
                      </button>
                      <button
                        ...
                        disabled={!hasNext}
                        onClick={() =>
                          onCopyNextFrame(selectedAnim.id, frame.id)
                        }
                      >
                        Next →
                      </button>
                  ...
                  <div className={styles.frameRow}>
                    ...
                    <span className={styles.readonlyVal}>
                      {uv.u.toFixed(3)}, {uv.v.toFixed(3)} · {uv.uw.toFixed(3)}×
                      {uv.vh.toFixed(3)}
                    </span>
                  </div>
                </div>
