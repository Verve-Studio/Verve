...
export function DialogButtonRow({
  ...
  onCancel,
  onAddSwatch,
}: DialogButtonRowProps): React.JSX.Element {
  return (
...
function labToRgb(L: number, a: number, bv: number): [number, number, number] {
  ...
  return [
    delinearize(Math.max(0, rl)),
    delinearize(Math.max(0, gl)),
    delinearize(Math.max(0, bl)),
  ];
}
...
function drawGradient(
  ...
  for (let py = 0; py < H; py++) {
    ...
    for (let px = 0; px < W; px++) {
      ...
      switch (mode) {
        case "S":
          [pr, pg, pb] = hsvToRgb(u * 360, s, t);
          break; // x=hue y=val
        case "B":
          [pr, pg, pb] = hsvToRgb(u * 360, t, v);
          break; // x=hue y=sat
        case "R":
          pr = r;
          pg = u;
        ...
        case "LL": {
          const [lL] = rgbToLab(r, g, b);
          [pr, pg, pb] = labToRgb(lL, u * 254 - 127, t * 254 - 127);
          break;
        } // x=a, y=b
        case "La": {
          const [, la] = rgbToLab(r, g, b);
          [pr, pg, pb] = labToRgb(t * 100, la, u * 254 - 127);
          break;
        } // x=b, y=L
        case "Lb": {
          const [, , lb] = rgbToLab(r, g, b);
          [pr, pg, pb] = labToRgb(t * 100, u * 254 - 127, lb);
          break;
        } // x=a, y=L
      }
      const i = (py * W + px) * 4;
      d[i] = Math.round(pr * 255);
      d[i + 1] = Math.round(pg * 255);
      d[i + 2] = Math.round(pb * 255);
      d[i + 3] = 255;
    }
  }
...
function drawStrip(
  ...
  for (let py = 0; py < H; py++) {
    ...
    switch (mode) {
      case "S":
        [pr, pg, pb] = hsvToRgb(h, t, v);
        break; // S varies top→bottom
      case "B":
        [pr, pg, pb] = hsvToRgb(h, s, t);
        break; // V varies
      case "R":
        pr = t;
        pg = g;
      ...
      case "LL": {
        const [, la, lbv] = rgbToLab(r, g, b);
        [pr, pg, pb] = labToRgb(t * 100, la, lbv);
        break;
      }
      case "La": {
        const [lL, , lbv] = rgbToLab(r, g, b);
        [pr, pg, pb] = labToRgb(lL, t * 254 - 127, lbv);
        break;
      }
      case "Lb": {
        const [lL, la] = rgbToLab(r, g, b);
        [pr, pg, pb] = labToRgb(lL, la, t * 254 - 127);
        break;
      }
    }
    for (let px = 0; px < W; px++) {
...
function getGradXY(
  ...
  switch (mode) {
    ...
    case "LL":
      return [(la + 127) / 254, 1 - (lbv + 127) / 254];
    case "La":
      return [(lbv + 127) / 254, 1 - lL / 100];
    case "Lb":
      return [(la + 127) / 254, 1 - lL / 100];
  }
}
...
export function ColorPickerDialog({
    ...
    (e: React.PointerEvent<HTMLCanvasElement>): void => {
      ...
      switch (mode) {
        ...
        case "R": {
          const ng = u,
            nb = 1 - y;
          const [nh, ns, nv] = rgbToHsv(r, ng, nb);
          setHue(nh);
          setSat(ns);
          setVal(nv);
          break;
        ...
        case "G": {
          const nr = u,
            nb = 1 - y;
          const [nh, ns, nv] = rgbToHsv(nr, g, nb);
          setHue(nh);
          setSat(ns);
          setVal(nv);
          break;
        ...
        case "Bl": {
          const nr = u,
            ng = 1 - y;
          const [nh, ns, nv] = rgbToHsv(nr, ng, b);
          setHue(nh);
          setSat(ns);
          setVal(nv);
          break;
        ...
        case "LL": {
          const [lL] = rgbToLab(r, g, b);
          const [nr, ng, nb] = labToRgb(lL, u * 254 - 127, (1 - y) * 254 - 127);
        ...
        case "La": {
          const [, la] = rgbToLab(r, g, b);
          const [nr, ng, nb] = labToRgb((1 - y) * 100, la, u * 254 - 127);
          const [nh, ns, nv] = rgbToHsv(nr, ng, nb);
          setHue(nh);
          setSat(ns);
          setVal(nv);
        ...
        case "Lb": {
          const [, , lbv] = rgbToLab(r, g, b);
          const [nr, ng, nb] = labToRgb((1 - y) * 100, u * 254 - 127, lbv);
          const [nh, ns, nv] = rgbToHsv(nr, ng, nb);
          setHue(nh);
          setSat(ns);
          setVal(nv);
  ...
  const onStripPointer = useCallback(
    (e: React.PointerEvent<HTMLCanvasElement>): void => {
      if (e.type === "pointerdown")
        e.currentTarget.setPointerCapture(e.pointerId);
      if (!(e.buttons & 1)) return;
      ...
      switch (mode) {
        ...
        case "R": {
          const nr = 1 - y;
          const [nh, ns, nv] = rgbToHsv(nr, g, b);
          setHue(nh);
          setSat(ns);
          setVal(nv);
          break;
        ...
        case "G": {
          const ng = 1 - y;
          const [nh, ns, nv] = rgbToHsv(r, ng, b);
          setHue(nh);
          setSat(ns);
          setVal(nv);
          break;
        ...
        case "Bl": {
          const nb = 1 - y;
          const [nh, ns, nv] = rgbToHsv(r, g, nb);
          setHue(nh);
          setSat(ns);
          setVal(nv);
          break;
        ...
        case "LL": {
          const [, la, lbv] = rgbToLab(r, g, b);
          const [nr, ng, nb] = labToRgb((1 - y) * 100, la, lbv);
          const [nh, ns, nv] = rgbToHsv(nr, ng, nb);
          setHue(nh);
          setSat(ns);
          setVal(nv);
        ...
        case "La": {
          const [lL, , lbv] = rgbToLab(r, g, b);
          const [nr, ng, nb] = labToRgb(lL, (1 - y) * 254 - 127, lbv);
          const [nh, ns, nv] = rgbToHsv(nr, ng, nb);
          setHue(nh);
          setSat(ns);
          setVal(nv);
        ...
        case "Lb": {
          const [lL, la] = rgbToLab(r, g, b);
          const [nr, ng, nb] = labToRgb(lL, la, (1 - y) * 254 - 127);
          const [nh, ns, nv] = rgbToHsv(nr, ng, nb);
          setHue(nh);
          setSat(ns);
          setVal(nv);
  ...
  const setH = (v: number): void => {
    setHue(Math.max(0, Math.min(360, v)));
  };
  const setS = (v: number): void => {
    setSat(Math.max(0, Math.min(100, v)) / 100);
  };
  const setV = (v: number): void => {
    setVal(Math.max(0, Math.min(100, v)) / 100);
  };
  const setR = (v: number): void => {
    const [nh, ns, nv] = rgbToHsv(clamp01(v), g, b);
    setHue(nh);
    setSat(ns);
    setVal(nv);
  };
  const setG = (v: number): void => {
    const [nh, ns, nv] = rgbToHsv(r, clamp01(v), b);
    setHue(nh);
    setSat(ns);
    setVal(nv);
  };
  const setB = (v: number): void => {
    const [nh, ns, nv] = rgbToHsv(r, g, clamp01(v));
    setHue(nh);
    setSat(ns);
    setVal(nv);
  };
  ...
  const setLabL = (v: number): void => {
    const [nr, ng, nb] = labToRgb(Math.max(0, Math.min(100, v)), labA, labB);
    const [nh, ns, nv] = rgbToHsv(nr, ng, nb);
    setHue(nh);
    setSat(ns);
    setVal(nv);
  };
  const setLabA = (v: number): void => {
    const [nr, ng, nb] = labToRgb(labL, Math.max(-127, Math.min(127, v)), labB);
    const [nh, ns, nv] = rgbToHsv(nr, ng, nb);
    setHue(nh);
    setSat(ns);
    setVal(nv);
  };
  const setLabB = (v: number): void => {
    const [nr, ng, nb] = labToRgb(labL, labA, Math.max(-127, Math.min(127, v)));
    const [nh, ns, nv] = rgbToHsv(nr, ng, nb);
    setHue(nh);
    setSat(ns);
    setVal(nv);
  };
  const setCmykC = (v: number): void => {
    const [nr, ng, nb] = cmykToRgb(
          ...
          <div className={styles.hsvSliders}>
            ...
            <div className={styles.hsvRow}>
              ...
              <input
                ...
                className={styles.hsvSlider}
                style={{
                  background: `linear-gradient(to right, rgb(${hsvToRgb(
                    hue,
                    0,
                    val,
                  )
                    .map((v) => Math.round(v * 255))
                    .join(",")}), rgb(${hsvToRgb(hue, 1, val)
                    .map((v) => Math.round(v * 255))
                    .join(",")}))`,
                }}
            ...
            <div className={styles.hsvRow}>
              ...
              <input
                ...
                className={styles.hsvSlider}
                style={{
                  background: `linear-gradient(to right, #000, rgb(${hsvToRgb(
                    hue,
                    sat,
                    1,
                  )
        ...
        <div className={styles.rightPanel}>
          ...
          <div className={styles.preview}>
            ...
            <div className={styles.previewSwatch}>
              ...
              />
              <div
                className={styles.previewCurrent}
              ...
              <label className={styles.fieldRow}>
                ...
                {pixelFormat === "rgba32f" ? (
                  ...
                  <input
                    ...
                    min={0}
                    max={255}
                    value={Math.round(r * 255)}
                    onChange={(e) => setR(+e.target.value / 255)}
                  />
                )}
              </label>
              ...
              <label className={styles.fieldRow}>
                ...
                {pixelFormat === "rgba32f" ? (
                  ...
                  <input
                    ...
                    min={0}
                    max={255}
                    value={Math.round(g * 255)}
                    onChange={(e) => setG(+e.target.value / 255)}
                  />
                )}
              </label>
              ...
              <label className={styles.fieldRow}>
                ...
                {pixelFormat === "rgba32f" ? (
                  ...
                  <input
                    ...
                    min={0}
                    max={255}
                    value={Math.round(b * 255)}
                    onChange={(e) => setB(+e.target.value / 255)}
                  />
                )}
              </label>
