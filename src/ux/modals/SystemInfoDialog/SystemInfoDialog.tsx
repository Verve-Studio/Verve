...
export function SystemInfoDialog({
  open,
  onClose,
}: SystemInfoDialogProps): React.JSX.Element | null {
      ...
      if ("gpu" in navigator) {
        try {
          ...
          if (adapter) {
            ...
            webGpuAdapter =
              info?.description || info?.device || info?.architecture || null;
  ...
  return (
    ...
    >
            ...
            <div className={styles.section}>
              ...
              <div className={styles.row}>
                ...
                <span className={styles.value}>
                  {formatBytes(data.totalRamBytes)}
                </span>
              </div>
            </div>
                ...
                data.gpus.map((g, i) => (
                  <div key={i} className={styles.row}>
                    ...
                    <span className={styles.value}>
                      {g.name}
                      {g.active && (
                        <span className={styles.gpuBadge}>Active</span>
                      )}
                    </span>
                  </div>
                ))
              ...
              <div className={styles.row}>
                ...
                <span className={styles.value}>
                  {data.webGpuVendor && data.webGpuAdapter
                    ...
                    : data.webGpuAdapter ||
                      data.webGpuVendor ||
                      (data.gpus.find((g) => g.active)?.name ??
                        "Not available")}
                </span>
              </div>
              <div className={styles.row}>
      ...
      <div className={styles.footer}>
        <DialogButton width="196px" align="center" onClick={onClose} primary>
          Close
        </DialogButton>
      </div>
    </ModalDialog>
