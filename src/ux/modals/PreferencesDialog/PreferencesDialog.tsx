...
function MemorySection(): React.JSX.Element {
  ...
  const cappedBytesUsed = prefs.unifiedMemory
    ? cpuBytesUsed + gpuBytesUsed
    : cpuBytesUsed;
  ...
  const handleBufferMaxOutChange = (
    ...
    <div className={styles.sectionBody}>
      <div className={styles.field}>
        ...
        <p className={styles.hint}>
          Maximum RAM the undo / redo history is allowed to use. When this limit
          is reached, the oldest entries are discarded to make room for new
          ones. Lower values keep memory usage in check on large documents;
          higher values give you more undo steps to work with.
        </p>
      </div>
      ...
      <div className={styles.field}>
        ...
        <p className={styles.hint}>
          Soft cap on memory used by all open documents — layer pixel data, GPU
          textures, and compositing buffers. Operations that would push usage
          past this limit are aborted with an error so the app stays responsive
          instead of crashing the OS. Enable
          <strong> Max Out</strong> to disable the cap and let allocations run
          until the operating system itself fails (
          {systemRamBytes != null
            ? `installed RAM: ${formatBytes(systemRamBytes)}`
      ...
      <div className={styles.field}>
        ...
        <p className={styles.hint}>
          Auto-detected from your platform on first launch — leave this on for
          Apple Silicon and other systems with integrated graphics, off for
          machines with a dedicated GPU and separate VRAM.
        </p>
      </div>
      ...
      <div className={styles.field}>
        <span className={styles.fieldLabel}>
          ...
          {!prefs.bufferMemoryMaxOut && (
            <>
              {" "}
              — {formatBytes(cpuBytesUsed)} of{" "}
              {formatBytes(prefs.bufferMemoryBytes)}
            </>
          )}
          {prefs.bufferMemoryMaxOut && (
            <> — {formatBytes(cpuBytesUsed)} (uncapped)</>
          )}
        </span>
        <div className={styles.usageBar}>
          <div
      ...
      <div className={styles.field}>
        <span className={styles.fieldLabel}>
          ...
          {(!prefs.unifiedMemory || prefs.bufferMemoryMaxOut) && (
            <>
              {" "}
              — {formatBytes(gpuBytesUsed)}{" "}
              {prefs.unifiedMemory ? "(uncapped)" : "(VRAM, uncapped)"}
            </>
          )}
        </span>
        <div className={styles.usageBar}>
              ...
              width: `${
                prefs.bufferMemoryMaxOut
                  ...
                  : prefs.unifiedMemory
                    ? Math.min(
                        100,
                        (gpuBytesUsed / prefs.bufferMemoryBytes) * 100,
                      )
                    : // Discrete: scale against the largest GPU footprint we've
                      ...
                      Math.min(
                        100,
                        (gpuBytesUsed /
                          Math.max(gpuBytesUsed, prefs.bufferMemoryBytes)) *
                          100,
                      )
              }%`,
            }}
        ...
        <span className={styles.hint}>
          ...
          adjustment / filter caches). On dedicated-VRAM systems these live in
          VRAM and aren't capped by the buffer-memory limit.
        </span>
      </div>
      ...
      <div className={styles.field}>
        <span className={styles.fieldLabel}>
          ...
          {!prefs.bufferMemoryMaxOut && prefs.unifiedMemory && (
            <>
              {" "}
              of {formatBytes(prefs.bufferMemoryBytes)} (
              {Math.round(bufferPctUsed)}% of cap)
            </>
          )}
        </span>
        <div className={styles.usageBar}>
  ...
  return (
    <ModalDialog open={open} title="Preferences" width={620} onClose={onClose}>
      ...
      <div className={styles.footer}>
        <DialogButton width="196px" onClick={onClose}>
          Close
        </DialogButton>
      </div>
    </ModalDialog>
