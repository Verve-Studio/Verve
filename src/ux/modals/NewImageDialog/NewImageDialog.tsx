...
function ClipboardIcon(): React.JSX.Element {
  return (
    ...
    >
      ...
      />
      <path
        d="M4.5 3.5V2.5a.5.5 0 0 1 .5-.5h3a.5.5 0 0 1 .5.5v1"
        stroke="currentColor"
        strokeWidth="1.2"
      />
    </svg>
...
export function NewImageDialog({
  ...
  onConfirm,
  onCancel,
}: NewImageDialogProps): React.JSX.Element | null {
  ...
  const handlePpiChange = useCallback(
    (newPpi: number): void => {
      ...
      if (unit !== "px") {
        setWidthPx((prev) =>
          Math.max(1, Math.min(16384, Math.round((prev * newPpi) / ppi))),
        );
        setHeightPx((prev) =>
          Math.max(1, Math.min(16384, Math.round((prev * newPpi) / ppi))),
        );
  ...
  const physLabel =
    unit !== "px" ? `${dispW} × ${dispH} ${UNIT_LABEL[unit]}` : null;
      ...
      <div className={styles.body}>
        ...
        <div className={styles.detailsPanel}>
          ...
          <div className={styles.fieldRow}>
            <label className={styles.fieldLabel} htmlFor="ni-width">
              Width
            </label>
          ...
          {/* Height */}
          <div className={styles.fieldRow}>
            <label className={styles.fieldLabel} htmlFor="ni-height">
              Height
            </label>
            <div className={styles.inputGroup}>
              <input
          ...
          {/* Units */}
          <div className={styles.fieldRow}>
            <label className={styles.fieldLabel} htmlFor="ni-unit">
              Units
            </label>
            <select
              id="ni-unit"
          ...
          {/* Resolution */}
          <div className={styles.fieldRow}>
            <label className={styles.fieldLabel} htmlFor="ni-res">
              Resolution
            </label>
            <div className={styles.inputGroup}>
              <input
                ...
                onChange={(e) => {
                  const v = e.target.valueAsNumber;
                  if (!isNaN(v) && v >= 1) handlePpiChange(v);
                }}
              />
          ...
          {/* Color Mode */}
          <div className={styles.fieldRow}>
            <label className={styles.fieldLabel} htmlFor="ni-colormode">
              Color Mode
            </label>
            <select
              id="ni-colormode"
          ...
          <div className={`${styles.fieldRow} ${styles.fieldRowBg}`}>
            ...
            <div className={styles.bgGroup}>
              {(["white", "black", "transparent"] as BackgroundFill[]).map(
                (fill) => (
                  <label
                    key={fill}
